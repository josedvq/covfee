...
covfee reads its configuration from the folder in which it is run (ie. the project folder). When run in deployment mode, covfee will look for `deployment.env` in that folder.

### Basic configuration

If you are deploying covfee in a typical server, add a `deployment.env` file like the following to your project folder:
...
COVFEE_ADMIN_PASSWORD=admin
```

Covfee loads this file with `python-dotenv` and then reads values with Flask `from_prefixed_env()`. Most keys therefore use the `COVFEE_` prefix. The one exception that looks a little redundant is `COVFEE_COVFEE_SECRET_KEY`, which maps to Covfee's internal `COVFEE_SECRET_KEY` setting.

The `BASE_URL` option is derived from `COVFEE_HOST` and `COVFEE_PORT`.
