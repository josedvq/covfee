...
First create a folder for the covfee project. This folder may be anywhere in the file system. Then create the following structure:
...
This starts webpack, the Flask backend, the Node Redux store service, and the standalone `www` fileserver together. After this the Covfee admin panel should be accessible in the URL displayed when running the backend process.
