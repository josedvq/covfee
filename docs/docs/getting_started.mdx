...
Here again we want to provide a local video to covfee by placing it in the `www` folder. You can [download this video](https://cdn.jsdelivr.net/gh/josedvq/covfee@nonseq-buffer/samples/keypoints/www/person_reading.mp4) to use as a sample. 
...
:::info
By now your covfee specification (`example.covfee.json` file) should contain four tasks whithin its `tasks` section and look exactly [like this file](https://github.com/josedvq/covfee/blob/nonseq-buffer/samples/getting_started/example.covfee.json). Your project folder should look like this:
