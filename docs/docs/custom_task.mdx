...
To use the task in development, create a project folder with an `app.py` entrypoint:
...
Covfee looks for the module-level `app` object and uses it as the project entrypoint.

Then run the Procfile-based dev workflow from the main repository:
...
That starts webpack, the Flask backend, the Node Redux store service, and the standalone `www` fileserver together. The admin panel should then be available at the URL printed by the backend process.
