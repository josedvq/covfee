...
export const VideocallGUI: React.FC<Props> = (props) => {
  ...
  const numRows = Math.ceil(args.subscribers.length / numColumns)
  ...
  return (
    <GridContainer $rows={numRows} $columns={numColumns}>
      ...
      {!args.disabled && (
        <Bar>
          {args.allowMute && (
            <Button $active={args.muted} onClick={args.onMute}>
              <AudioOutlined />
            </Button>
          )}
          {args.allowStopVideo && (
            <Button $active={args.videoStopped} onClick={args.onStopVideo}>
              <VideoCameraOutlined />
            </Button>
          )}
          {args.allowEndCall && (
