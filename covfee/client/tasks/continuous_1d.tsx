...
interface State {}
export default class Continuous1DTask extends BaseTaskProps<Props, State> {
  static taskInfo = {
    bufferDataLen: 1,
  };
  ...
  componentDidMount() {
    this.props.buttons
      .addListener("play-pause", "Play/pause the video and data capture.")
      .addEvent("keydown", (e: Event) => {
        if (!this.props.response) {
          if (this.props.player.paused)
            this.props.buffer.log(this.props.player.currentTime() as number, [
              "play",
            ]);
          else
            this.props.buffer.log(this.props.player.currentTime() as number, [
  ...
  handleFrame = (time: number) => {
    if (this.props.response) {
      ...
      this.props.buffer.data(time, [this.intensity]);
    }
  };
  ...
  replayUntil = (time: number) => {
    ...
    if (data) this.setIntensity(data[2]);
  ...
  render() {
    if (
      ["ranktrace", "ranktrace-new", "gtrace"].includes(
        this.props.spec.intensityInput.mode
      )
    )
      ...
      [
        ...
        "gravity-keyboard",
        "binary",
      ].includes(this.props.spec.intensityInput.mode)
    )
