...
export default class ContinuousKeypointTask extends CovfeeContinuousTask<
  Props,
  State
> {
  static taskInfo = {
  ...
  constructor(props: Props) {
    super(props);
    let opticalFlowEnabled = !!props.spec.media.hasFlow;
    if (typeof localStorage !== "undefined") {
      opticalFlowEnabled = localStorage.getItem("opticalFlowEnabled") == "1";
    }

    // disable OF for replay
    if (props.response !== null) opticalFlowEnabled = false;

    this.state.opticalFlowEnabled = opticalFlowEnabled;

    log.info(`constructing task ${this.props.myKey}`);
    log.debug(
      `this.state.opticalFlowEnabled = ${this.state.opticalFlowEnabled}`
    );
  }
  ...
  componentDidMount() {
    ...
    this.props.buttons
      .addListener("speedup", "Increase playback speed.")
      .addEvent("keydown", (e: Event) => {
        this.speedup();
      });

    this.props.buttons
      .addListener("speeddown", "Decrease playback speed.")
      .addEvent("keydown", (e: Event) => {
        this.speeddown();
      });

    this.props.buttons
      .addListener("play", "Toggle play/pause the video.")
      .addEvent("keydown", (e: Event) => {
        this.props.player.togglePlayPause();
      });

    this.props.buttons
      .addListener("back2s", "Go back 2s.")
      .addEvent("keydown", (e: Event) => {
        this.back2s();
      });

    this.props.buttons
      .addListener("back10s", "Go back 10s.")
      .addEvent("keydown", (e: Event) => {
        this.back10s();
      });

    this.props.buttons
      .addListener("occlusion", "Toggle occlusion flag.")
      .addEvent("keydown", (e: Event) => {
        this.toggleOcclusion();
      });

    this.props.buttons
      .addListener("opticalflow", "Toggle optical flow-based speed adjustment.")
      .addEvent("keydown", (e: Event) => {
  ...
  speedup = () => {
    this.setState({
      playbackRateIdx: Math.min(
        this.state.playbackRateIdx + 1,
        this.playbackRates.length - 1
      ),
    });
  };
  ...
  speeddown = () => {
    this.setState({
      playbackRateIdx: Math.max(this.state.playbackRateIdx - 1, 0),
    });
  };
  ...
  handleFrame = (time: number) => {
    if (this.props.response) {
      ...
      this.props.buffer.data(time, [
        ...this.mouse_normalized,
        this.state.mouse_valid ? 1 : 0,
        this.state.occluded ? 1 : 0,
      ]);
    }
  };
  ...
  replayUntil = (time: number) => {
    ...
    if (data) this.updateMouseVisualization([data[1], data[2]]);
    ...
    return (
      ...
      >
        <MouseVisualizer
          ...
          ref={(e) => {
            if (e) this.updateMouseVisualization = e.setData;
          }}
        >
          {elem}
        </MouseVisualizer>
      ...
      <>
        {this.props.renderPlayer({
          ...
          playbackRateMultiplier:
            this.playbackRates[this.state.playbackRateIdx],
