...
import { DownOutlined, LinkOutlined, NodeIndexOutlined } from "@ant-design/icons";
...
export const HitBlock = (props: Props) => {
  ...
  const [collapsed, _setCollapsed] = React.useState<boolean>(props.instance.collapsed)
  const [showGraph, _setShowGraph] = React.useState<boolean>(props.instance.show_graph)
  const [showJourneys, _setShowJourneys] = React.useState<boolean>(props.instance.show_journeys)
  const [showNodes, _setShowNodes] = React.useState<boolean>(props.instance.show_nodes)
  const {
    hitInstance, 
  ...
  return (
    <Container>
      <Header onClick={()=>{setCollapsed(!collapsed)}}>
        ...
        <NodeIndexOutlined /> {`${hitInstance.journeys.filter(j => j.submitted).length} / ${hitInstance.journeys.length}`}
      ...
      {!collapsed &&
        <div>
          ...
          <SectionButton onClick={()=>setShowJourneys(!showJourneys)}>Journeys</SectionButton>
          ...
          <SectionButton onClick={()=>setShowNodes(!showNodes)}>Nodes</SectionButton>
...
const JourneyRow = (props: JourneyType) => {
  ...
  return <div>
    <a href={journey.getUrl()}><span>{props.id.substring(0,10)}</span> <LinkOutlined /></a>
  </div>
}
...
const NodeRow = (props: NodeType) => {
  ...
  return <div>
    <a href={node.getUrl()}><LinkOutlined /></a>
  </div>
}