...
const AdminProject = (props: Props) => {
    ...
    const [currentProjectIndex, setCurrentProjectIndex] = React.useState<number>(null)
    ...
    React.useEffect(()=> {
        ...
        getProject(projects[currentProjectIndex].id)
            .then(proj => {
                setProject(proj)
    ...
    switch(state.status) {
        ...
        case 'ready':
            return <>
                ...
                <Modal visible={state.hitEditor.visible} footer={null} onCancel={()=>{setState({...state, hitEditor:{...state.hitEditor, visible: false}})}}>
                    <HitEditorForm
                ...
                <div style={{margin: '2em 1em'}}>
                    Project: <Select 
                        ...
                        onChange={i=>{setCurrentProjectIndex(i)}} 
                        style={{ width: 240 }}>
                        {projects.map((p, index) => {
                            return <Option key={index} value={index}>{p.name}</Option>
                        })}
                    </Select>
                    <Button type='primary' href={Constants.api_url + '/projects/' + project.id + '/csv'}>Download URLs</Button>
                    <Button href={Constants.api_url + '/projects/' + project.id + '/download?csv=1'}>Download results (CSV)</Button>
                </div>
